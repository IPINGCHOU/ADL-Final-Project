...
class Network(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, atom_size: int, support: torch.Tensor):
        ...
        self.feature_layer = nn.Sequential(
            nn.Conv2d(in_dim, 32, kernel_size=8, stride=3),
            nn.BatchNorm2d(32),
            nn.ReLU(),
...
class ReplayBuffer:
    ...
    def __init__(self, obs_dim: tuple, size: int, batch_size: int = 32, n_step: int = 1, gamma: float = 0.99):
...
class PrioritizedReplayBuffer(ReplayBuffer):
    ...
    def __init__(self, obs_dim: tuple, size: int, batch_size: int = 32, alpha: float = 0.6, n_step: int = 1, gamma: float = 0.99,):
        """Initialization."""
        assert alpha >= 0
